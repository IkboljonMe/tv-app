...
import com.karuhun.feature.onboarding.presentation.component.VideoPlayer
...
fun OnBoardingScreen(
    ...
    uiEffect.collectWithLifecycle { effect ->
        when (effect) {
            is OnBoardingContract.UiEffect.ShowError -> {
            }
        }
    }
        ...
        uiState.videoConfig?.let { videoConfig ->
            VideoPlayer(
                ...
                onError = { errorMessage ->
                    onAction(OnBoardingContract.UiAction.OnVideoError(errorMessage))
                },
                onBufferingChanged = { isBuffering ->
                    onAction(OnBoardingContract.UiAction.OnBufferingStateChanged(isBuffering))
                },
            )
        }
...
fun OnBoardingScreenPreview() {
    val uiState = OnBoardingContract.UiState(
        isLoading = false,
    ...
    OnBoardingScreen(uiState = uiState, uiEffect = emptyFlow(), onAction = {})
}